...
> 보관 상태: 날짜별 작업 기록을 도입하기 전 사용한 누적 문서다. 이후 기록은 `docs/ai-usage/YYYY-MM-DD/`에 작업별 파일로 작성한다.
...
## 2026-08-16 전투 후속 객체지향 리팩터링 M1

- 사용한 AI 도구/모델: Codex, GPT-5 계열 모델
- 사용자 요청: 기존 전투 동작과 공개 API를 유지하면서 `UnitManager`의 남은 책임을 분리하고, 진화 선택 UI와 이벤트 기반 전멸 판정 및 자동 포션 처리를 복구
- AI 제안 내용: 생성·준비/합성·아이템 책임을 각각 `UnitSpawnController`, `UnitPreparationController`, `UnitItemController`로 분리하고 Manager는 roster와 시스템 조정만 담당
- AI 실제 수정 영역: 전투 유닛 Controller 3개와 Unity meta 추가, `UnitManager` 위임 구조 및 진화 선택 이벤트 복구, `BattleManager` 전멸 이벤트와 결과 대기 Coroutine, 아군 피해 기반 다음 프레임 자동 포션 예약, `EvolutionPanel` 바인딩 실패 취소 경로
- 사용자 직접 결정/수정 필요 영역: 진화 선택은 필수이며 선택 중 준비 행동 전체 잠금, UnitSpawner의 필요 시 생성 후 SetActive 재사용 유지, 전멸 즉시 판정과 다음 프레임 자동 포션 정책을 사용자 결정으로 확정
- 중요한 프롬프트/지시: 별도 워크트리 사용, 승인된 설계 범위만 구현, 테스트·빌드·Unity 실행·정적 분석·검사를 위한 검사 금지
- 테스트/검증 결과: 사용자 지침에 따라 실행형 검증은 수행하지 않았고, 변경된 호출 관계와 이벤트 순서만 코드 읽기로 대조함
