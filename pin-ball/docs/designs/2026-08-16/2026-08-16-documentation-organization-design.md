...
`.github`에는 GitHub/Copilot이 직접 읽는 `copilot-instructions.md`와 현재 규칙의 원문인 `project-master-prompt.md`만 유지한다.

- `프로젝트 지침서.md`, `아트 리소스 지침서.md`: `docs/handbook`으로 이동
- `추가 인계 지침서.md`: `docs/handbook/새_채팅_추가_인계_지침서.md`와 중복이므로 canonical 문서만 유지
- `ai-use-log.md`: 날짜별 기록 도입 전 누적 자료이므로 `docs/ai-usage/legacy`로 이동
- `.github/plans`: `docs/plans`의 해당 날짜 폴더로 이동

## 루트 유지 문서

- `AGENTS.md`: Agent가 저장소 진입 시 읽는 고정 규칙이므로 이동하지 않는다.
- `.github/project-master-prompt.md`: 기존 프로젝트 지침 위치를 유지한다.
- `.github/copilot-instructions.md`: GitHub Copilot이 직접 읽는 위치를 유지한다.

## 내용 갱신

M2~M8의 최신 설계 문서에 `구현 상태`를 추가한다. 구현 완료 여부와 설계/구현 커밋을 사실대로 기록한다. M6에서 의도적으로 유지한 Button 자동 검색과 M8의 Tutorial PlayerPrefs 유지 정책도 최종 결정으로 명시한다. 루트 `AGENTS.md`, Copilot 지침과 원문 프로젝트 프롬프트에는 검증 제한과 문서 저장 위치를 동기화한다.
