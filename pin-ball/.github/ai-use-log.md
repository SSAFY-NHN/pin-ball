...
## 2026-08-26 아군 준비 배치 영역 및 카메라 이동 경계 수정

- 사용한 AI 도구/모델: Codex, GPT-5 계열 모델
- 사용자 요청: 아군 스폰·드래그 영역을 화면에 표시한 파란 영역과 빨간 영역의 교집합 정도로 제한하고, 카메라 이동 시 영역이 오른쪽으로 밀리는 간헐 문제도 수정
- AI 제안 내용: 교집합을 `Panel_BattleArea`의 명시적 RectTransform 영역으로 지정하고, 아군 배치 계산이 그 전체 영역을 사용하게 하며 카메라 위치 변경 시 화면 좌표 기반 월드 경계를 갱신
- AI 실제 수정 영역: Game 씬 `Panel_BattleArea` 가로 앵커, `BattleAreaBounds`의 아군 포함·클램프·격자 계산과 카메라 이동 감지, 아군 준비 배치 EditMode 회귀 테스트
- 사용자 직접 결정/수정 필요 영역: 사용자가 파란색과 빨간색의 교집합 정도를 목표 범위로 결정했으며, 실제 Game View에서 좌우 경계 체감에 따라 앵커 `0.23`과 `0.57`을 미세 조정할 수 있음
- 중요한 프롬프트/지시: 기존 구조 보존, 최소 수정, 씬 배치 우선, 스폰과 드래그가 같은 경계 사용, 카메라 이동 문제 포함
- 테스트/검증 결과: 변경 diff와 공백 오류 검사는 통과했다. Unity EditMode 테스트는 로컬 Licensing Client 연결 실패로 완료되지 않았고, 대체 `dotnet build`도 샌드박스의 Microsoft SDK 경로 접근 거부로 실행되지 않았다. 실제 Game View에서 카메라 전환 전후 스폰·드래그 범위 확인이 필요하다.

## 2026-08-26 적군 웨이브 스폰 위치 조정

- 사용한 AI 도구/모델: Codex, GPT-5 계열 모델
- 사용자 요청: 웨이브 시작 시 적군이 아군 바로 앞에 소환되는 문제를 정적인 스폰 위치 수정으로 해결
- AI 제안 내용: Game 씬의 `EnemySpawnPoint`를 전투 화면 왼쪽으로 이동하고, 전투 경계 밖에서 생성된 적군이 첫 프레임에 아군 영역 경계로 강제 보정되지 않게 처리
- AI 실제 수정 영역: Game 씬 `EnemySpawnPoint`의 X 좌표를 `-6.59`에서 `-8`로 변경하고, `UnitBase` 이동 보정을 현재 위치가 전투 경계 안일 때만 적용
- 사용자 직접 결정/수정 필요 영역: 실제 Game View에서 적군 등장 여백과 아군까지의 이동 거리 체감 확인
- 중요한 프롬프트/지시: 정적 수정, 기존 구조와 대형 유지, 최소 변경
- 테스트/검증 결과: 사용자 요청에 따라 자동 테스트 없이 변경 내용을 확인함. 실제 플레이 체감 확인은 사용자 확인 항목으로 남김.
